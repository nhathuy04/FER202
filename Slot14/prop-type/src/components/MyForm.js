import React, { useReducer, useEffect, useState } from "react"; // Thêm React và các hook cần thiết
...
const MyForm = ({ title, onSubmit }) => {
  ...
  const [errors, setErrors] = useState({}); // useState được định nghĩa sau khi import React
  const [showAlert, setShowAlert] = useState(false); // useState được định nghĩa sau khi import React

  // Kiểm tra lỗi khi component mount và khi state thay đổi
  useEffect(() => {
    handleValidation();
  }, [state]);
