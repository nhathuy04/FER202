...
const QuizPage = () => {
  ...
  const { history, questions, status, error } = useSelector((state) => state.quiz);
  ...
  useEffect(() => {
    if (courses.length > 0 && courseId && course) {
      dispatch(fetchQuestions(courseId));
    }
  }, [dispatch, courseId, courses, course]);


  const handleStartQuiz = () => {
    navigate(`/quiz/start/${courseId}`);
  };
  ...
  if (status === 'loading') return <div className="text-center py-5">Đang tải...</div>;
  if (status === 'failed') return <Alert variant="danger" className="text-center">{error}</Alert>;
    ...
    <Container className="my-4">
      <Row>
        <Col md={6}>
          <Card>
            <Card.Body>
              <Card.Title>Quiz {course?.name || 'Khóa học'} (Tổng: {questions.length} câu)</Card.Title>
              <p><strong>Tổng số câu hỏi:</strong> {questions.length}</p>
              <p><strong>Mô tả:</strong> Đây là Quiz về {course?.name || 'khóa học'}.</p>
              ...
              <Form>
                ...
                <Form.Group className="mb-3">
                  <Form.Check
                    ...
                    disabled={!isVerified || questions.length === 0}
                  />
                </Form.Group>
                <Button
                  variant="primary"
                  disabled={!agree || !isVerified || questions.length === 0}
